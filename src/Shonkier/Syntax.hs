...
singleton :: String -> Val' a -> Env' a
singleton x v y = v <$ guard (x == y)

merge :: Env -> Env -> Env
merge rho sig x = sig x <|> rho x

match :: Pattern -> Val -> Maybe Env
match (PAtom a)   (VAtom b)   = init <$ guard (a == b)
...
matches ps vs = foldl merge init <$> mayZipWith match ps vs
